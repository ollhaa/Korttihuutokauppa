...
def update_final():
    try:
        ...
        WHERE auctions.solved = False AND auctions.active = False AND auctions.user_id = users.id"
        ...
        for x in todo2:
            admin =1
            message = "Olet voittanut huutokaupan " + str(x[0]) + ": " + str(x[1]) + " (tai se on aloittamasi). Voit nyt ottaa yhteyttä mailiin: " + str(x[4]) +"."
            sql3 = """INSERT INTO messages (user_id_from, user_id_to, message, message_sent) VALUES (:user_id_from, :user_id_to, :message, NOW())"""
            db.session.execute(text(sql3),{"user_id_from":admin, "user_id_to":x[3], "message":message})
            message2 = "Tekemäsi huutokauppa on päättynyt"
            sql4 = """INSERT INTO messages (user_id_from, user_id_to, message, message_sent) VALUES (:user_id_from, :user_id_to, :message, NOW())"""
            db.session.execute(text(sql4),{"user_id_from":admin, "user_id_to":x[2],"message":message2})
        #print("täällä?")
...
def make_bid(auction_id, bid):
    ...
    try: 
        ...
        if len(result.fetchone()) == 0:
            ...
            sql = """INSERT INTO bids (user_id, auction_id, bid, bid_time) VALUES (:user_id, :auction_id, :bid, NOW())"""
            db.session.execute(text(sql), { "user_id":user_id, "auction_id":auction_id, "bid":new_price})
            db.session.commit()
    except:
        return False
